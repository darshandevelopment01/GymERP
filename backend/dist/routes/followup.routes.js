...
const autoExpireFollowUps = async () => {
    try {
        ...
        const overdueFollowUps = await FollowUp_1.default.find({
            status: 'pending',
        ...
        if (expiredIds.length > 0) {
            await FollowUp_1.default.updateMany({ _id: { $in: expiredIds } }, { $set: { status: 'expired' } });
...
router.post('/', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const populatedFollowUp = await FollowUp_1.default.findById(followUp._id)
...
router.get('/member/:memberId', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        await autoExpireFollowUps();
        const followUps = await FollowUp_1.default.find({ member: req.params.memberId })
            .populate('createdBy', 'name')
            .sort({ createdAt: -1 });
        res.json({
            success: true,
...
router.get('/enquiry/:enquiryId', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        await autoExpireFollowUps();
        const followUps = await FollowUp_1.default.find({ enquiry: req.params.enquiryId })
            .populate('createdBy', 'name')
            .sort({ createdAt: -1 });
        res.json({
            success: true,
...
router.patch('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        // ✅ Fetch old followup before update
        const oldFollowUp = await FollowUp_1.default.findById(req.params.id).lean();
        const followUp = await FollowUp_1.default.findByIdAndUpdate(req.params.id, { status }, { new: true })
            .populate('member', 'name memberId mobileNumber')
            .populate('enquiry', 'name mobileNumber email'); // ✅ Added enquiry population
        if (!followUp) {
            return res.status(404).json({ message: 'Follow-up not found' });
...
router.delete('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const followUp = await FollowUp_1.default.findByIdAndDelete(req.params.id);
        if (!followUp) {
            return res.status(404).json({ message: 'Follow-up not found' });
        }
        res.json({
